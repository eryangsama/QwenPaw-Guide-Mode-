...
class QwenPawAgent(ToolGuardMixin, ReActAgent):
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _register_hooks(self) -> None:
        ...
        # Workspace hooks - auto-discover and load from hooks/ directory
        ...
        if hooks_dir.is_dir():
            for hook_file in sorted(hooks_dir.glob("*.py")):
                ...
                try:
                    ...
                    # Register all Hook classes found in the module
                    for attr_name in dir(module):
                        attr = getattr(module, attr_name)
                        if (
                            isinstance(attr, type)
                            and attr_name.endswith("Hook")
                            and attr_name != "BootstrapHook"
                        ):
                            hook_instance = attr()
                            # Detect hook type: first look for named methods,
                            # fall back to __call__ as post_reply
                            ...
                            for hook_type in (
                                ...
                                if hasattr(hook_instance, hook_type):
                                    hook_fn = getattr(hook_instance, hook_type)
                                    self.register_instance_hook(
                                        hook_type=hook_type,
                                        hook_name=f"workspace_{attr_name}_{hook_type}",
                                        hook=hook_fn,
                                    )
                                    logger.debug(
                                        f"Registered workspace hook "
                                        f"{attr_name}::{hook_type}"
                                    )
                                    hook_registered = True
                            # Fallback: if only __call__ is defined, treat as post_reply
                            if not hook_registered and hasattr(
                                hook_instance, "__call__"
                            ):
                                self.register_instance_hook(
                                    hook_type="post_reply",
                                    hook_name=f"workspace_{attr_name}_post_reply",
                                    hook=hook_instance.__call__,
                                )
                                logger.debug(
                                    f"Registered workspace hook "
                                    f"{attr_name}::post_reply (via __call__)"
                                )
                except Exception as e:
                    logger.warning(
                        f"Failed to load workspace hook {hook_file.name}: {e}"
                    )
    ...
    async def _force_guidance_action_if_text_only(
        ...
        hint_body = (
            ...
            "guidance. If no tool action is actually required, answer "
            "briefly.\n"
            "</system-hint>"
        )
        if guidance_hint:
